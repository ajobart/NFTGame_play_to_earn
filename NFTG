...
contract NFTGAME is ERC721, Ownable {
    ...
    struct Character {
        ...
        uint life;
        uint32 experience;
        // Attributs pour savoir quand le personnage s'est heal et combattu pour la dernière fois
        uint lastHeal;
        uint lastFight;
        // Pour ce souvenir du type de character
    ...
    function mint(type_character _typeCharacter) public {
        // L'utilisateur ne doit pas pouvoir créer + de 5 persos
        // balanceof est une fonctionnalité dispo dans le contrat 'ERC721' qui permet de retourner le nombre de NFT dont l'utilisateur dispose
        ...
        if(_typeCharacter == type_character.ORCS) {
            ...
            // Fonctionnalité dans le protocole ERC721 qui permet de 'minter' de façon safe en étant sur que ce soit bien réalisé
            _safeMint(msg.sender, nextId);
            // On incrémente pour que la prochaine création est un numéro d'id supérieur
            nextId++;
        }
    ...
    function fight(uint _tokenId1, uint _tokenId2) public payable {
        ...
        // On vérifie que la personne qui détient le _tokenId1 soit bien la personne qui call la fonction
        require(ownerOf(_tokenId1) == msg.sender, "This is not your character");
        // On vérifie que la personne qui détient le _tokenId1 ne soit pas aussi le propriétaire du _tokenId2
        require(ownerOf(_tokenId1) != ownerOf(_tokenId2), "You can't fight your own character");
        // On vérifie que la vie des deux personnages soit > 0 donc qu'il ne sont pas mort
